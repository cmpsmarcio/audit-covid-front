          ...
          return( 
            <UserList key={index}>
              ...
              <div>
                ...
                <Label>{`Teve contato com alguém infectado recentemente: ${booleanToTextPtBr(data.covidData.contactWithInfectedRecently)}`}</Label>
                <Label>{`Já teve Covid 19: ${booleanToTextPtBr(data.covidData.alreadyInfected)}`}</Label>
                <Label>{`Já foi vacinado: ${booleanToTextPtBr(data.covidData.alreadyVaccinated)}`}</Label>
              </div>
            </UserList>)
        })
      }
